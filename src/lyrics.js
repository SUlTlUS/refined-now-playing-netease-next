...
export function Lyrics(props) {
	...
	const getLiveCurrentTimeRaw = useCallback(() => {
		...
		return currentTime.current + elapsed;
	}, []);

	const getLiveCurrentTime = useCallback(() => {
		return getLiveCurrentTimeRaw() + _globalOffset.current;
	}, [getLiveCurrentTimeRaw]);
...
function Line(props) {
	...
	const karaokeLineBaseTime = props.line.dynamicLyricTime ?? props.line.time ?? 0;
	const karaokeLineRef = useRef(null);
	const getDisplayedCurrentTime = useCallback(() => {
		return props.getCurrentTime ? props.getCurrentTime() : props.currentTime;
	}, [props.getCurrentTime, props.currentTime]);
	const effectiveCurrentTime = props.currentLine === props.id ? getDisplayedCurrentTime() : props.currentTime;
	const getWordAbsoluteTime = (word) => {
		const wordTime = word?.time ?? 0;
		// Netease YRC usually stores word timestamps as absolute song time.
		// Keep relative providers working by only adding the line base for small offsets.
		if (wordTime >= karaokeLineBaseTime - 1000) {
			return wordTime;
		}
		return karaokeLineBaseTime + wordTime;
	};
	const getWordDuration = (word) => Math.max(1, word?.duration ?? 0);
	const getWordProgress = (word, currentTimeValue = effectiveCurrentTime) => {
		return clamp((currentTimeValue - getWordAbsoluteTime(word)) / getWordDuration(word));
	};
	const karaokeAnimationFloat = (word) => {
		...
		const wordDelay = getWordAbsoluteTime(word) - effectiveCurrentTime;
		...
		return {
			transitionDuration: `${wordDuration}ms, ${wordDuration + 150}ms`,
	...
	const karaokeAnimationSlide = (word) => {
		...
		return {
			transitionDuration: `${wordDuration}ms, ${wordDuration * 0.8}ms, 0.5s`,
			transitionDelay: `${wordDelay}ms, ${wordDelay + wordDuration * 0.5}ms, 0ms`,
		};
	};
	const karaokeFillerAnimationSlide = (word) => {
		if (props.currentLine != props.id) {
			return {
				transitionDuration: '0ms, 0ms, 0.5s',
				transitionDelay: '0ms',
		...
		if (props.playState == false) {
			return {
				...
				WebkitMaskPositionX: `${100 * (1 - progress)}%`,
				transform: 'none',
			};
		}
		return {
	...
	const getKaraokeAnimation = (word) => {
		...
		if (props.karaokeAnimation == 'slide') {
			return karaokeAnimationSlide(word);
		}
		return {};
	};
	const getKaraokeFillerAnimation = (word) => {
		if (props.karaokeAnimation == 'slide') {
			return karaokeFillerAnimationSlide(word);
		}
		return {};
	};
	...
	useEffect(() => {
		...
		const glowAnimation = (index) => {
			...
			const glowAnimationTiming = {
				...
				fadeAway: fadeAway,
				duration: duration,
				wordTime: getWordAbsoluteTime(props.line.dynamicLyric[index]),
	...
	useEffect(() => {
		...
		for (let glowAnimation of glowAnimationsRef.current) {
			...
			if (props.playState == false) {
				...
				animation.currentTime = effectiveCurrentTime - timing.wordTime;
				...
				{props.line.dynamicLyric.map((word, index) => {
					return <div
						...
						style={getKaraokeAnimation(word)}>
							<span>{word.word}</span>
							{
								props.karaokeAnimation == 'slide' && <span className="rnp-karaoke-word-filler" style={getKaraokeFillerAnimation(word)}>{word.word}</span>
							}
					</div>
				})}
			</div> }
...
function Interlude(props) {
	...
	const effectiveCurrentTime = props.currentLine === props.id ? renderTime : props.currentTime;
	...
	return (
		<div
			className={`rnp-interlude-inner ${props.currentLine != props.id || props.playState == false ? 'pause-breath' : ''}`}
			ref={dotContainerRef}>
			{dots.map((dot, index) => {
				return <div
					key={index}
